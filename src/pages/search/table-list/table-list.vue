<template>
	...
	<div class="i-table-no-border">
		...
		<Alert show-icon class="ivu-mt">
			<div v-font="14">
				已选择 <strong v-color="'#2d8cf0'">{{ selectedData.length }}</strong>总计 <strong>{{ totalSelectedCount }}分</strong>
		...
		<Table ref="table" :columns="columns" :data="dataWithPage" :loading="loading" class="ivu-mt" @on-sort-change="handleSortChange"
		 ...
		 @on-select-all="handleSelectAll" @on-select-all-cancel="handleSelectAllCancel">
			...
			<template slot-scope="{ row }" slot="status">
				<Badge v-if="row.status === 0" status="success" text="已公示" />
				<Badge v-if="row.status === 1" status="error" text="未公示" />
			</template>
			<template slot-scope="{ row }" slot="statuss">
				<Badge v-if="row.statuss === 0" statuss="success" text="是" />
				<Badge v-if="row.statuss === 1" statuss="error" text="否" />
			</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			mockOneData() {
				const item = {};
				item.createtime = new Date((new Date()).getTime() - Math.floor(Math.random() * 8640000000));
				item.overtime = new Date((new Date()).getTime() - Math.floor(Math.random() * 8640000000));
				item.name = random(6);
				item.count = Math.floor(Math.random() * 10 + 40);
				item.status = Math.floor(Math.random() * 2);
				item.statuss = Math.floor(Math.random() * 2);
				item._checked = false;
				return item;
			},
			getData() {
