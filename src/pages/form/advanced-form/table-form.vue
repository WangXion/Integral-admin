<template>
	<Form ref="form" :model="data" :rules="rules" :label-width="labelWidth" :label-position="labelPosition">
		<Row :gutter="24" type="flex" justify="end">
			<Col v-bind="grid">
			<FormItem label="姓名搜索" prop="name" label-for="name">
				<Input v-model="data.name" placeholder="请输入" element-id="name" />
			</FormItem>
			</Col>
			<Col v-bind="grid">
			<FormItem label="村搜索" prop="status1" label-for="status1">
				<Input v-model="data.cun" placeholder="请输入" element-id="cun" />
			</FormItem>
			</Col>
			<Col v-bind="grid">
			<FormItem label="组搜索" prop="status2" label-for="statu2">
				<Input v-model="data.zu" placeholder="请输入" element-id="zu" />
			</FormItem>
			</Col>
			<Col v-bind="grid">
			<FormItem label="年龄搜索" prop="status3" label-for="status3">
				<Input v-model="data.age" placeholder="请输入" element-id="age" />
			</FormItem>
			</Col>
